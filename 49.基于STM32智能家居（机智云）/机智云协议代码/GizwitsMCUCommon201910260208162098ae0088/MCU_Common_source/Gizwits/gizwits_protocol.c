...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagFS)
    {
        info->event[info->num] = EVENT_FS;
        info->num++;
        dataPoints->valueFS = gizX2Y(FS_RATIO,  FS_ADDITION, issuedData->attrVals.valueFS); 
    }
    ...
    if(0x01 == issuedData->attrFlags.flagPM_2_5)
    {
        info->event[info->num] = EVENT_PM_2_5;
        info->num++;
        dataPoints->valuePM_2_5 = gizX2Y(PM_2_5_RATIO,  PM_2_5_ADDITION, exchangeBytes(issuedData->attrVals.valuePM_2_5));
    }
        
    if(0x01 == issuedData->attrFlags.flagM5_Value)
    {
        info->event[info->num] = EVENT_M5_Value;
        info->num++;
        dataPoints->valueM5_Value = gizX2Y(M5_Value_RATIO,  M5_Value_ADDITION, exchangeBytes(issuedData->attrVals.valueM5_Value));
    }
        
    if(0x01 == issuedData->attrFlags.flagM135_Val)
    {
        info->event[info->num] = EVENT_M135_Val;
        info->num++;
        dataPoints->valueM135_Val = gizX2Y(M135_Val_RATIO,  M135_Val_ADDITION, exchangeBytes(issuedData->attrVals.valueM135_Val));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuePM_2_5 != cur->valuePM_2_5)
    {
        GIZWITS_LOG("valuePM_2_5 Changed\n");
        ret = 1;
    }
    if(last->valueM5_Value != cur->valueM5_Value)
    {
        GIZWITS_LOG("valueM5_Value Changed\n");
        ret = 1;
    }
    if(last->valueM135_Val != cur->valueM135_Val)
    {
        GIZWITS_LOG("valueM135_Val Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueFS = gizY2X(FS_RATIO,  FS_ADDITION, dataPoints->valueFS); 

    devStatusPtr->valuePre = exchangeBytes(gizY2X(Pre_RATIO,  Pre_ADDITION, dataPoints->valuePre)); 
    devStatusPtr->valuePM_2_5 = exchangeBytes(gizY2X(PM_2_5_RATIO,  PM_2_5_ADDITION, dataPoints->valuePM_2_5)); 
    devStatusPtr->valueM5_Value = exchangeBytes(gizY2X(M5_Value_RATIO,  M5_Value_ADDITION, dataPoints->valueM5_Value)); 
    devStatusPtr->valueM135_Val = exchangeBytes(gizY2X(M135_Val_RATIO,  M135_Val_ADDITION, dataPoints->valueM135_Val)); 
